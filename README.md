...
1. **Двухпротокольный анти-блокировочный стек**:
   ...
   - **Режим ЧС / Обход «Белых списков» (Covert Channels)**:
     - Технология прямого обхода шатдаунов и изоляции Рунета (на базе идей OpenFlux).
     - Инкапсуляция зашифрованного TCP/IP трафика (ChaCha20-Poly1305) в события совместного редактирования **Yandex Docs (WebSocket cursor messages)** и комнаты **Cups.online (Centrifugo channels)**.
     - Пробивает сеть даже когда все зарубежные IP полностью отрезаны и работают только российские сайты из белого списка.
...
## 🚨 Запуск Режима ЧС («Обход белых списков / OpenFlux»)

Если зарубежный интернет заблокирован и работают только сервисы из «Белого списка» (Яндекс, VK и др.), используется встроенный инструмент `covert`:

### 1. На зарубежном сервере (Exit Node):
```bash
./covert \
  --mode exit \
  --transport cups \
  --room "секретный_uuid_комнаты" \
  --key "32-байтный_секретный_ключ"
```
*(Или `--transport yandex` с параметрами документа Яндекс Документов)*

### 2. На компьютере пользователя (Клиент):
```bash
./covert \
  --mode client \
  ...
  --key "32-байтный_секретный_ключ"
```
После этого на компьютере поднимается локальный SOCKS5-прокси на `127.0.0.1:1080`. Весь трафик идет через белый список отечественного сервиса и выходит в свободный интернет через зарубежный VPS.
